...
function check() {
    ...
    if (letter != '') {
        ...
        for (let i = 0; i < word.length; i++) {
            if (word[i] == letter) {
                document.getElementById(`let${i}`).textContent = letter;
                isRight = true;
            }
            if (document.getElementById(`let${i}`).textContent == '?')
                isAllLetersOpened = false;
        }
        if (!isRight && countOfSteps != 0 && !wrong.includes(letter)) {
            ...
            document.getElementById('image').innerHTML = `<img src="img${10 - countOfSteps}.png" alt="Виселица">`;
        }
        else if (wrong.includes(letter)) {
            alert("Такая буква уже была");
        }
        else if (countOfSteps == 0) {
